        ...
        <CardContent className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1">
            ...
            <select name="following_scraper_provider" defaultValue={initial.following_scraper_provider}
              className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm">
              <option value="auto">Auto (Apify, fallback to ScrapingBee)</option>
              <option value="apify">Apify only</option>
              <option value="scrapingbee">ScrapingBee only</option>
            </select>
            <p className="text-xs text-muted-foreground">ScrapingBee requires an IG session cookie below.</p>
          </div>
          <div className="space-y-1 col-span-2">
            <Label className="text-sm">Instagram session cookie</Label>
            ...
            <p className="text-xs text-muted-foreground">
              Burner-account cookie. When set, metadata backfill runs <b>free</b> (direct IG fetch) instead of via Apify.
              Also used by ScrapingBee for following-list fallback. Save first, then click below to verify.
            </p>
            <TestCookieButton />
          </div>
        </CardContent>
